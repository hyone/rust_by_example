...
impl error::Error for DoubleError {
    fn description(&self) -> &str {
        match *self {
            // A very short description of the error.
            // it doesn't need to be the same as `Display`.
            DoubleError::EmptyVec     => "empty vectors not allowed",
            // `e` already impls `Error`, so defer to its own implementation.
    ...
    fn cause(&self) -> Option<&error::Error> {
        match *self {
            // No underlying error, so return `None`.
